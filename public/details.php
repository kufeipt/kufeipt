...
JS;
        ...
        // ------------- 开始复制种子链接 ------------------//
        tr($lang_details['torrent_dl_url'],sprintf("<span id=\"content\">
        ". getSchemeAndHttpHost()."/download.php?downhash=".$CURUSER['id']."|".$torrentRep->encryptDownHash($row['id'], $CURUSER) ."
        </span> <br/> <button id=\"copyBT\">点击复制种子链接</button>"),1);
        ?>
        <script>
 ...
 //            $subTorrentIdArr = array_merge($subTorrentIdArr, $otherCopiesIdArr);
       ...
       //  print("<td class=\"embedded\"><form method=\"get\" action=\"https://assrt.net/sub/\" target=\"_blank\"><input type=\"text\" name=\"searchword\" id=\"keyword\" style=\"width: 250px\" value=\"".$moviename."\" /><input type=\"submit\" value=\"".$lang_details['submit_search_at_shooter']."\" /></form></td><td class=\"embedded\"><form method=\"get\" action=\"https://www.opensubtitles.org/en/search2/\" target=\"_blank\"><input type=\"hidden\" id=\"moviename\" name=\"MovieName\" /><input type=\"hidden\" name=\"action\" value=\"search\" /><input type=\"hidden\" name=\"SubLanguageID\" value=\"all\" /><input onclick=\"document.getElementById('moviename').value=document.getElementById('keyword').value;\" type=\"submit\" value=\"".$lang_details['submit_search_at_opensubtitles']."\" /></form></td>\n");
                ...
                switch ($imdb->getCacheStatus($imdb_id))
                {
                    ...
                    case "1" :
                    {
                        ...
                        if (($photo_url = $movie->photo() ) != FALSE)
                            $smallth = "<img src=\"".$photo_url. "\" width=\"100%\" onclick=\"Preview(this);\" alt=\"poster\" />";
                        else
                            $smallth = "<img src=\"pic/imdb_pic/nophoto.gif\" width=\"100%\" alt=\"no poster\" />";

                        $autodata = $imdb->renderDetailsPageDescription($row['id'], $imdb_id);
                        $cache_time = $imdb->getCachedAt($imdb_id);
                        $Cache->add_whole_row();
                        print("<tr>");
                        print("<td class=\"rowhead\"><a href=\"javascript: klappe_ext('imdb')\"><span class=\"nowrap\"><img class=\"minus\" src=\"pic/trans.gif\" alt=\"Show/Hide\" id=\"picimdb\" title=\"".$lang_details['title_show_or_hide']."\" /> ".$lang_details['text_imdb'] . $lang_details['row_info'] ."</span></a></td>");
                        $Cache->end_whole_row();
                        $Cache->add_row();
                        $Cache->add_part();
                        print("<td class=\"rowfollow\" align=\"left\"><div id='kimdb' style=\"width:50%;float:left\">".$autodata);
                        $Cache->end_part();
                        $Cache->add_part();
                        ...
                        $Cache->end_row();
                        $Cache->add_whole_row();
                        print("</div><div id=\"posterimdb\" style=\"float:left;width:34%\">".  $smallth."</div></td></tr>");
                        $Cache->end_whole_row();
                        $Cache->cache_page();
                        echo $Cache->next_row();
                        $Cache->next_row();
        ...
        //print("<tr><td class=\"rowhead\">"."<font color=\"red\">*</font></td><td class=\"rowfollow\">");
        //print("<textarea class=\"bbcode\" cols=\"100\" style=\"width: 100%;\" name=\"descr\" id=\"descr\" rows=\"20\" onkeydown=\"ctrlenter(event,'compose','qr')\">".$row["descr"]."</textarea>");
        //textbbcode("edittorrent","descr",($row["descr"]), false, false, false);
        //print("</td></tr>");
        //technical info
        ...
        <tr>
            <td class="rowhead nowrap" valign="top" align="right">
                <a href="javascript: klappe_news('othercopynew')">
                    <span class="nowrap">
                        <img class="plus" src="pic/trans.gif" alt="Show/Hide" id="picothercopynew" title="显示&nbsp;或&nbsp;隐藏"/> 简介代码
                    </span>
                </a>
            </td>
            <td class="rowfollow" valign="top" align="left">
                <span>展开后点击按钮复制</span>
                <div id='kothercopynew' style="display: none;">
                    <button  onclick="copy()">一键复制简介代码</button>
                    <br/>
                    <br/>
                    <textarea class="foo" rows="20" cols="115%" id="write"><?php print($row["descr"]); ?></textarea>
            </td>
            </div>
        </tr>
        <script>
