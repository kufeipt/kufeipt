...
if(isset($_GET['type']) && $_GET['type']){
    $myopt = $_GET['type'];
    if($myopt == 'keep'){
        $wherea[] = "seeders<5 and seeders>0";
        $addparam .= "type=keep&";
    }elseif($myopt == 'anime'){
        $wherea[] = "category in (417,418,419,420,421)";
        $addparam .= "type=anime&";
    }elseif($myopt == 'audio'){
        $wherea[] = "category in (423,424)";
        $addparam .= "type=audio&";
    }elseif($myopt == 'pc'){
        $wherea[] = "category in (425,426)";
        $addparam .= "type=pc&";
    }elseif($myopt == 'other'){
        $wherea[] = "category in (409)";
        $addparam .= "type=other&";
    }elseif($myopt == 'movie'){
        $wherea[] = "category in (401,402,403,422)";
        $addparam .= "type=movie&";
    }
}
$special_state = 0;
...
if ($tagId > 0) {
    ...
    $myopt = $_GET['config'];
    if($myopt == 'tags'){
        $tagFilter = " inner join torrent_tags on torrents.id = torrent_tags.torrent_id and torrent_tags.tag_id on (3,12)";
        $addparam .= "config=tags";
    }
    }
}
...
if($inclbookmarked == 1)
{
	print("<h1 align=\"center\">" . get_username($CURUSER['id']) . /*$lang_torrents['text_s_bookmarked_torrent'] . */"个人RSS订阅地址</h1>");
//RSS订阅筐链接
	print("<h1 align=\"center\"><a href=https://kufei.org/torrentrss.php?passkey=".$CURUSER['passkey']."&rows=10&paid=0&linktype=dl&inclbookmarked=1 style=\"font-size:15px;text-align:center;color:red;\">https://kufei.org/torrentrss.php?passkey=".$CURUSER['passkey']."&rows=10&paid=0&linktype=dl&inclbookmarked=1</a></h1>");
}
...
$run_id = $xhprof_runs->save_run($xhprof_data, "torrents");
