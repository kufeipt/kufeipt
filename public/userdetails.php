...
if (user_can('userprofile') ||  $user["id"] == $CURUSER["id"])
{
	...
	else $locationinfo = "<span title=\"" . $loc_mod . "\">[" . $loc_pub . "]</span>";
...
if (mysql_num_rows($res) > 0)
{
	...
	{
	    list($loc_pub4, $loc_mod4) = get_ip_location($arr['ipv4']);
	    list($loc_pub6, $loc_mod6) = get_ip_location($arr['ipv6']);
	    $clientselect .= "<tr>";
		$clientselect .= sprintf('<td>%s</td>', get_agent($arr['peer_id'], $arr['agent']));
		if (user_can('userprofile') ||  $user["id"] == $CURUSER["id"]) {
            $clientselect .= sprintf('<td>%s</td><td>%s</td><td>%s</td>', $arr['ipv4'].$loc_pub4.$seedBoxRep->renderIcon($arr['ipv4'], $user['id']), $arr['ipv6'].$loc_pub6.$seedBoxRep->renderIcon($arr['ipv6'], $user['id']), $arr['port']);
        } else {
